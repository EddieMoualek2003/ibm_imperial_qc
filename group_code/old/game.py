...
def draw_play_mode():
    ...
    SCREEN.blit(title, (WIDTH // 2 - title.get_width() // 2, 20))
    score_text = f"Player 1: {scores['Player 1']}    {'Player 2' if game_mode == '2players' else 'Computer'}: {scores['Player 2' if game_mode == '2players' else 'Computer']}"
...
def draw_winner_screen():
    ...
    if scores["Player 1"] == scores[opponent]:
        msg = "It's a tie!"
    else:
        winner = "Player 1" if scores["Player 1"] > scores[opponent] else opponent
        msg = f"{winner} wins!"
    winner_text = FONT.render(msg, True, RED)
    SCREEN.blit(winner_text, (WIDTH // 2 - winner_text.get_width() // 2, 200))
    restart_msg = FONT.render("Click anywhere to play again", True, BLACK)
...
def handle_demo_click(qid):
    global current_demo_step, message, state
...
def create_qubits_with_qiskit():
    ...
    while len(available_indices) >= 2:
        q1 = available_indices.pop()
        q2 = available_indices.pop()
